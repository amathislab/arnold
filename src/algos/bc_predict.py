...
from models.ppo.policies import PredictiveMuscleTransformerPolicy

@dataclasses.dataclass(frozen=True)
class BCPredictTrainingMetrics(BCTrainingMetrics):

    obs_prediction_loss: torch.Tensor

@dataclasses.dataclass(frozen=True)
class BCPredictLossCalculator:
    ...
    def __call__(
        self,
        policy: PredictiveMuscleTransformerPolicy,
        obs: Union[
            types.AnyTensor,
            types.DictObs,
            Dict[str, np.ndarray],
        ...
        _, log_prob, entropy, next_obs_pred = policy.evaluate_actions_and_predict_next_obs(tensor_obs, acts)
        ...
        loss = neglogp + ent_loss + l2_loss  + next_obs_pred_loss

        return BCPredictTrainingMetrics(
            neglogp=neglogp,
            entropy=entropy,
            ent_loss=ent_loss,
            ...
            l2_norm=l2_norm,
            l2_loss=l2_loss,
            obs_prediction_loss=next_obs_pred_loss,
            loss=loss,
        )
...
def make_predict_transition_data_loader(
...
class BCPredict(algo_base.DemonstrationAlgorithm) :
    ...
    def __init__(
        ...
        **unused_kwargs: Any,
        ...
        self.loss_calculator = BCPredictLossCalculator(ent_weight, l2_weight, obs_pred_weight)
    ...
    def set_demonstrations(self, demonstrations: algo_base.AnyTransitions) -> None:
        self._demo_data_loader = make_predict_transition_data_loader(
            demonstrations,
            self.minibatch_size,
        )
    ...
    def train(
        ...
        *,
        ...
        for (
            ...
            training_metrics = self.loss_calculator(self.policy, obs_tensor, next_obs_tensor, acts)

...
class BCValueTrainingMetrics(BCTrainingMetrics):

    l2_action_loss: torch.Tensor
    value_loss: torch.Tensor
    value_mean_loss: torch.Tensor
    value_dmean_loss: torch.Tensor
    real_value_mean: torch.Tensor
    real_value_std: torch.Tensor
    predicted_value_mean: torch.Tensor
    predicted_value_std: torch.Tensor

@dataclasses.dataclass(frozen=True)
class BCValueLossCalculator :
    ...
    def __call__(
        ...
        value = util.safe_to_tensor(value)
        ...
        pred_value = policy.predict_values(tensor_obs)
        value = value.to(pred_value.device)
        value_mean = value.mean()
        value_dmean = value - value_mean
        pred_value_mean = pred_value.mean()
        pred_value_dmean = pred_value - pred_value_mean
        value_dmean_loss = torch.mean(torch.square(value_dmean - pred_value_dmean))
        value_mean_loss = (value_mean - pred_value_mean)**2
        value_loss = (value_dmean_loss + value_mean_loss) * self.value_weight
        ...
        loss = l2_action_loss + neglogp + ent_loss + l2_loss + value_loss

        return BCValueTrainingMetrics(
            neglogp=neglogp,
            ...
            value_mean_loss=value_mean_loss,
            value_dmean_loss=value_dmean_loss,
            real_value_mean=value_mean,
            real_value_std=value.std(),
            predicted_value_mean=pred_value_mean,
            predicted_value_std=pred_value.std(),
            loss=loss,
        )

def rew_transitions_collate_fn(
...
def make_rew_transition_data_loader(
    ...
    if isinstance(transitions, types.TransitionsMinimal):
        ...
        return algo_base._WrappedDataLoader(transitions, batch_size)  # type: ignore[arg-type]
    else:
        raise TypeError(f"`demonstrations` unexpected type {type(transitions)}")


class BCValue(algo_base.DemonstrationAlgorithm) :
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        self.loss_calculator = BCValueLossCalculator(ent_weight, l2_weight, value_weight, action_loss_mode)
    ...
    def set_demonstrations(self, demonstrations: algo_base.AnyTransitions) -> None:
        self._demo_data_loader = make_rew_transition_data_loader(
            demonstrations,
            self.minibatch_size,
        )
    ...
    def train(
        ...
        *,
        ...
        for (
            ...
            training_metrics = self.loss_calculator(self.policy, obs_tensor, acts, value)
